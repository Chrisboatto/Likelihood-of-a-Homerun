...
# Create the plot and assign it to an object (recommended)
p <- ggplot(baseImp, aes(x = row.names(baseImp), y = IncNodePurity)) + 
  ...
  coord_flip()  # Optional but recommended for horizontal bars with readable labels

print(p)

# Save the plot to a specific file location
ggsave(filename = "C:/Users/cboat/OneDrive/Documents/GitHub/Likelihood-of-a-Homerun/Base Model Importance.png", 
       plot = p, 
...
newImp <- data.frame(
  Variable = rownames(newImp),
  IncNodePurity = newImp[, "IncNodePurity"]  # adjust if column name differs slightly
)

# Sort descending for nicer plot
newImp <- newImp[order(newImp$IncNodePurity, decreasing = TRUE), ]
# Optional: reset row names so they don't interfere
rownames(newImp) <- NULL

# === FIXED PLOT CODE ===
p <- ggplot(newImp, aes(x = Variable, y = IncNodePurity)) +   # ← Use Variable here!
  geom_bar(stat = "identity", width = 0.1, fill = "black") +
  geom_point(shape = 21, size = 3, colour = "black", fill = "red", stroke = 2) +
  labs(title = "Home Run Importance", x = "Importance", y = "Variable") +
  coord_flip() +   # Makes variable names readable on the y-axis
...
# Save it
ggsave(filename = "C:/Users/cboat/OneDrive/Documents/GitHub/Likelihood-of-a-Homerun/New Model Importance.png", 
       plot = p, 
       width = 8, height = 6, dpi = 300, device = "png")
```
